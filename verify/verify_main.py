...
client_perc = 1 / client

# Image dataset
trainset = Cifar10(root="data/", download=True, train=True, unlearning=False, img_size=32, augment= True)
testset = Cifar10(root="data/", download=True, train=False, unlearning=False, img_size=32, augment= False)

# split in backdoor and clean data
backdoor_trainset, clean_trainset = torch.utils.data.random_split(
    trainset, [client_perc, 1 - client_perc])

backdoor_testset, clean_testset = torch.utils.data.random_split(
    testset, [client_perc, 1 - client_perc])



print("Type backdoor trainset:", type(backdoor_trainset))

trigger_label = 0
trigger_size = 5

backdoor_trainset, backdoor_pertubbed_trainset, backdoor_trainset_true = image_backdoor(dataset=backdoor_trainset, trigger_size= trigger_size, trigger_label= trigger_label, unlearn_mode= "single", sigma= 0.5)

backdoor_testset, backdoor_pertubbed_testset, backdoor_testset_true = image_backdoor(dataset=backdoor_testset, trigger_size= trigger_size, trigger_label= trigger_label, unlearn_mode= "single", sigma= 0.5)


#create training data from confidence vectors

#split unlearning client in train and test data
split_train_size = int(len(backdoor_trainset)*0.9)
split_test_size = int(len(backdoor_testset)*0.9)
train_client_traindata = backdoor_trainset[:split_train_size]
print("train ", len(train_client_traindata))
test_client_traindata = backdoor_trainset[split_train_size:]
print("test ", len(test_client_traindata))
train_client_testdata = backdoor_trainset[:split_test_size]
test_client_testdata = backdoor_trainset[split_test_size:]
...
print(len(backdoor_trainset))
print(len(clean_trainset))
print(len(backdoor_testset))
...
image, _, label = backdoor_trainset[178]
image = image.unsqueeze(0)
...
image, _, label = backdoor_trainset[179]
image = image.unsqueeze(0)
...
image, _, label = backdoor_trainset[180]
image = image.unsqueeze(0)
...
image, _, label = backdoor_trainset[181]
image = image.unsqueeze(0)
...
image, _, label = backdoor_trainset[182]
image = image.unsqueeze(0)
